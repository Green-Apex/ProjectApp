...
public class CustomAppBarLayout extends AppBarLayout implements
    ...
    protected void onAttachedToWindow() {
        ...
        mParent = new WeakReference<CoordinatorLayout>(
                (CoordinatorLayout) getParent());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mBehavior == null) {
            mBehavior = (Behavior) ((CoordinatorLayout.LayoutParams) getLayoutParams())
                    .getBehavior();
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (r - l > 0 && b - t > 0 && mAfterFirstDraw
    ...
    public void collapseToolbar(boolean withAnimation) {
        mQueuedChange = withAnimation ? ToolbarChange.COLLAPSE_WITH_ANIMATION
                : ToolbarChange.COLLAPSE;
        requestLayout();
    }
    ...
    public void expandToolbar(boolean withAnimation) {
        mQueuedChange = withAnimation ? ToolbarChange.EXPAND_WITH_ANIMATION
                : ToolbarChange.EXPAND;
        requestLayout();
    }

    private void performCollapsingWithoutAnimation() {
        if (mParent.get() != null) {
            mBehavior.onNestedPreScroll(mParent.get(), this, null, 0,
                    getHeight(), new int[]{0, 0});
        }
    }

    private void performCollapsingWithAnimation() {
        if (mParent.get() != null) {
            mBehavior.onNestedFling(mParent.get(), this, null, 0, getHeight(),
                    true);
        }
    }
    ...
    private void performExpandingWithAnimation() {
        if (mParent.get() != null) {
            mBehavior.onNestedFling(mParent.get(), this, null, 0,
                    -getHeight() * 5, false);
        }
    }
    ...
    public void onOffsetChanged(AppBarLayout appBarLayout, int i) {
        if (i == 0) {
            ...
            if (onStateChangeListener != null && state != State.HALF_EXPANDED) {
                onStateChangeListener.onStateChange(State.HALF_EXPANDED);
            }
            state = State.HALF_EXPANDED;
    ...
    public enum State {
        COLLAPSED, EXPANDED, HALF_EXPANDED, IDLE
    }
